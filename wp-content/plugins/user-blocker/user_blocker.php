    ...
    function myplugin_auth_signon($user, $username, $password) {
        if (!is_wp_error($user)) {
            ...
            if ($is_active == 'n') {
                ...
                return new WP_Error('authentication_failed', sprintf(__('<strong>ERROR</strong>:%s', 'user-blocker'), $block_msg_permenant));
            } else {
                $error_msg = '';
                ...
                if ($error_msg != '') {
                    return new WP_Error('authentication_failed', sprintf(__('<strong>ERROR</strong>:%s', 'user-blocker'), $error_msg));
                }
            }
        }
        return $user;
                            ...
                            if (isset($block_date) && !empty($block_date) && $block_date != '') {
                                ...
                                <tr>
                                    <td class="" colspan='7'>
                                        ...
                                        <?php echo $block_date['frmdate'] . ' ' . __('to', 'user-blocker') . ' ' . $block_date['todate']; ?>
                                    </td>
                                </tr>
                                <?php
                            }
    ...
    function wp_member_plugin_login_failed_sb_msg($str) {
        ...
        if ($_POST['log'] && $_POST['pwd']) {
            ...
            if (isset($user->errors['invalid_username'][0])) {
                $str = sprintf(__('%s', 'user-blocker'), $str);
                return $str;
            }
            if (isset($user->errors['authentication_failed'][0])) {
                $logfail = __('Login Failed!', 'user-blocker');
                $str = "<p class='err'>$logfail<br>" . sprintf(__('%s', 'user-blocker'), $user->errors['authentication_failed'][0]) . "</p>";
                return $str;
            }
        }
        return $str;
        ...
        if ($_POST['log'] && $_POST['pwd']) {
            ...
            if (isset($user->errors['authentication_failed'][0])) {
                ...
                $str = "<div align='center' id='wpmem_msg'>
                            <h2>$logfail</h2>
                            <p>" . sprintf(__('%s', 'user-blocker'), $user->errors['authentication_failed'][0]) . "</p>
                        </div>";
                return $str;
            }
        }
...
if (!function_exists('user_blocker_plugin_links')) {
    ...
    function user_blocker_plugin_links($links) {
        $links[] = '<a target="_blank" href="' . esc_url('https://www.solwininfotech.com/documents/wordpress/user-blocker/') . '">' . __('Documentation', 'user-blocker') . '</a>';
        return $links;
    }
